...
async def persembahan_add(user:user_refs,db:db_dependency, body:MoneyMoonUpdate):
    if not (body):
...
async def persembahan_update(user:user_refs,db:db_dependency,body:MoneyMoonUpdate):
